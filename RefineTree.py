...
"""
def Compute_Multiple_Avg_Clust_Dist_Score(clust_spec_list1, clust_spec_list2, DIST_METRIC):
  clust1_avg_dist = 0
  clust2_avg_dist = 0
  no_of_couplets = 0
  for x in clust_spec_list1:
	...
	continue
      ...
      if (DIST_METRIC == PROD_EXTRA_TAXA_BRANCH_COUNT):
  ...
  if (no_of_couplets == 0):
    ...
    return 0, 0, 1	# signifies that this couplet is not supported in the input trees
  return (clust1_avg_dist * 1.0) / no_of_couplets, (clust2_avg_dist * 1.0) / no_of_couplets, 0
...
def ResolveMultifurcation(Curr_tree, clust_species_list, no_of_input_clusters, NJ_RULE_USED, SUPPORT_COUPLET_CHECK, DIST_METRIC, CLASS_OF_METRIC, Output_Text_File):
  ...
  if (DIST_METRIC == PROD_EXTRA_TAXA_BRANCH_COUNT) or (DIST_METRIC == PROD_EXTRA_TAXA_ACC_RANK):
    Clust_DistMat1 = numpy.zeros((no_of_clust, no_of_clust), dtype=numpy.int)
  else:
  ...
  # we allocate extra distance matrix for the below mentioned distance measure
  if (DIST_METRIC == PROD_EXTRA_TAXA_BRANCH_COUNT) or (DIST_METRIC == PROD_EXTRA_TAXA_ACC_RANK):
  ...
  for i in range(no_of_clust - 1):
	...
	fp.close()
      ...
      if (DIST_METRIC == PROD_EXTRA_TAXA_BRANCH_COUNT) or (DIST_METRIC == PROD_EXTRA_TAXA_ACC_RANK):
	clust1_avg_dist, clust2_avg_dist, error_condition = Compute_Multiple_Avg_Clust_Dist_Score(clust_species_list[i], clust_species_list[j], DIST_METRIC)    
	if (DEBUG_LEVEL >= 2):
	  fp = open(Output_Text_File, 'a')
	  fp.write(' clust1_avg_dist: ' + str(clust1_avg_dist) + ' clust2_avg_dist: ' + str(clust2_avg_dist) + '  error: ' + str(error_condition))
	  fp.close()
	# store the cluster distance matrix data
	Clust_DistMat1[j][i] = Clust_DistMat1[i][j] = clust1_avg_dist
  ...
  while(no_of_clust > 2):      
	...
	if (DIST_METRIC == PROD_EXTRA_TAXA_BRANCH_COUNT) or (DIST_METRIC == PROD_EXTRA_TAXA_ACC_RANK):
	  PrintMatrixContent(no_of_clust, clust_species_list, Clust_DistMat2, 'Clust_DistMat2', Output_Text_File)  
    ...
    # add for the second distance matrix
    if (DIST_METRIC == PROD_EXTRA_TAXA_BRANCH_COUNT) or (DIST_METRIC == PROD_EXTRA_TAXA_ACC_RANK):
      sum_list = []
      for i in range(no_of_clust):
	t = 0
	for j in range(no_of_clust):
	  t = t + Clust_DistMat2[i][j]
	sum_list.append(t)
	...
	for j in range(i+1, no_of_clust):
	  if (NJ_RULE_USED == AGGLO_CLUST):
	    # this is agglomerative clustering
	    Norm_Clust_DistMat2[i][j] = ObtainNormalizedVal(Clust_DistMat2[i][j], sum_list[i], sum_list[j])
	    Norm_Clust_DistMat2[j][i] = Norm_Clust_DistMat2[i][j]
	  else:
	    # this is NJ based rule
	    ri = sum_list[i] / (no_of_clust - 2)
	    rj = sum_list[j] / (no_of_clust - 2)
	    Norm_Clust_DistMat2[i][j] = (Clust_DistMat2[i][j] - ri - rj)
	    Norm_Clust_DistMat2[j][i] = Norm_Clust_DistMat2[i][j]
	...
	fp.write('\n printing contents of sum_list --- ' + str(sum_list))
	fp.close()
	PrintMatrixContent(no_of_clust, clust_species_list, Norm_Clust_DistMat2, 'Norm_Clust_DistMat2', Output_Text_File)
    ...
    if (DIST_METRIC == PROD_EXTRA_TAXA_BRANCH_COUNT) or (DIST_METRIC == PROD_EXTRA_TAXA_ACC_RANK):
      Clust_DistMat1 = numpy.vstack((Clust_DistMat1, numpy.zeros((1, no_of_clust), dtype=numpy.int)))
      Clust_DistMat1 = numpy.hstack((Clust_DistMat1, numpy.zeros((no_of_clust + 1, 1), dtype=numpy.int)))
    else:
    ...
    for k in range(no_of_clust):
      ...
      if 0:	#(DIST_METRIC == EXTRA_TAXA):
	# use max XL heuristic as in AcRMNJXL technique
	Clust_DistMat1[no_of_clust][k] = max(Clust_DistMat1[min_idx_i][k], Clust_DistMat1[min_idx_j][k]) - 1
	# use max XL heuristic as used in the COSPEDSpec technique
	...
	# use average of elements located corresponding to min_idx_i and min_idx_j
	#Clust_DistMat1[k][no_of_clust] = (Clust_DistMat1[k][min_idx_i] + Clust_DistMat1[k][min_idx_j]) / 2 - 1
      
      # symmetric property maintain
      Clust_DistMat1[no_of_clust][k] = Clust_DistMat1[k][no_of_clust]
      
      if (DIST_METRIC == PROD_EXTRA_TAXA_BRANCH_COUNT) or (DIST_METRIC == PROD_EXTRA_TAXA_ACC_RANK):
	Clust_DistMat2[no_of_clust][k] = max(Clust_DistMat2[min_idx_i][k], Clust_DistMat2[min_idx_j][k]) - 1
    ...
    if (DIST_METRIC == PROD_EXTRA_TAXA_BRANCH_COUNT) or (DIST_METRIC == PROD_EXTRA_TAXA_ACC_RANK):
      Clust_DistMat2 = numpy.delete(Clust_DistMat2, (min_idx_i), axis=0)	# delete the row
    ...
    if (DIST_METRIC == PROD_EXTRA_TAXA_BRANCH_COUNT) or (DIST_METRIC == PROD_EXTRA_TAXA_ACC_RANK):
      Norm_Clust_DistMat2.fill(0)
      Norm_Clust_DistMat2 = numpy.delete(Norm_Clust_DistMat2, (min_idx_i), axis=0)	# delete the row
      Norm_Clust_DistMat2 = numpy.delete(Norm_Clust_DistMat2, (min_idx_i), axis=1)	# delete the column
